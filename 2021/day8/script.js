...
const byLen = (line, len) => [...line[0], ...line[1]].filter(e => [...len].includes(e.length))
...
4 5
 6
    */
    ...
    let sgm13 = strDiff(nr4, nr1), sgm46 = strDiff('abcdefg', seg[0]+nr1+sgm13);
...
const seg2Num = (on, map) => {
    ...
    '1111111',
    '1111011']
    let display = Array(7).fill(0);
    on.split('').map(l => display[map.indexOf(l)] = 1)
    return sevenSeg2Num.indexOf(display.join(''))
}

const part2 = (data, res = 0) => {
    data.map(item => {
        let seg = deduceSeg(item);
        res += parseInt(item[1].reduce((s, n) => s+seg2Num(n, seg) , ''))
    })
    return res;
}
