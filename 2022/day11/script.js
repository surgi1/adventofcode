...
const compute = (part2 = false, rounds = part2 ? 10000 : 20) => {
    ...
    const op = (old, opLit) => opLit[1] == '+' ? val(opLit[0], old)+val(opLit[2], old)
                                               : val(opLit[0], old)*val(opLit[2], old)
    ...
    const round = () => {
        monkeys.forEach(monkey => {
            while (monkey.stack.length) {
                ...
                if (!part2)
                    item = Math.floor((op(item, monkey.opLit) % mod) / 3);
                else
                    item = op(item, monkey.opLit) % mod;
