...
const run = (data, cols, rows, t = 100) => {
    ...
    data.forEach(([x, y, vx, vy]) => {
        x = modulo(x + vx*t, cols);
        y = modulo(y + vy*t, rows);
...
const run2 = (data, cols, rows) => {
    ...
    // for some reason it is rows*cols = 10403, I had to compute it though
    ...
    // surely there is a better way to compute think, modulo math basic knowledge (lack of) strikes again
    for (let n = 0; n < 10403; n++) {
        if (n % periodVert == startVert && n % periodHoriz == startHoriz) {
            t = n;
            break;
        }
    }
